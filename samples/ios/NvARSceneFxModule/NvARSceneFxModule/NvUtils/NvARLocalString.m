...
//        [NSBundle bundleForClass:[NvARLocalString class]];
...
//    NSLog(@"%@",CURR_LANG);
    if ([CURR_LANG containsString:@"en"]||[CURR_LANG containsString:@"zh"]) {
        s = NSLocalizedStringFromTableInBundle(translation_key, @"Localizable", [NSBundle bundleForClass:cls], comment);
    } else {
        NSString * path = [[NSBundle bundleForClass:cls] pathForResource:@"en" ofType:@"lproj"];
        NSBundle * languageBundle = [NSBundle bundleWithPath:path];
        s = [languageBundle localizedStringForKey:translation_key value:@"" table:nil];
    }
    return s;
