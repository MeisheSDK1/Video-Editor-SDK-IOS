...
@implementation NvVirtualGraphicBtn
...
+ (instancetype)buttonWithType:(UIButtonType)buttonType withTitle:(NSString *)title withImageNormal:(NSString *)normal withImageSelected:(NSString *)selected{
    NvVirtualGraphicBtn *btn = [NvVirtualGraphicBtn buttonWithType:buttonType];
    if (btn) {
        ...
        if (selected) {
            ...
            if (btn.isSelected) {
                btn.btnImageView.image = [NvBaseUtils imageNamed:selected inBundle:[NSBundle bundleForClass:[self class]]];
            }else{
                btn.btnImageView.image = [NvBaseUtils imageNamed:normal inBundle:[NSBundle bundleForClass:[self class]]];
            }
        }else{
            btn.btnImageView.image = [NvBaseUtils imageNamed:normal inBundle:[NSBundle bundleForClass:[self class]]];
        }
...
- (void)setCustomImage:(NSString *)image textColor:(NSString *)colorStr {
    self.btnImageView.image = [NvBaseUtils imageNamed:image inBundle:[NSBundle bundleForClass:[self class]]];
...
- (void)setSelected:(BOOL)selected{
    ...
    if (self.selectedString) {
        if (selected) {
            self.btnImageView.image = [NvBaseUtils imageNamed:self.selectedString inBundle:[NSBundle bundleForClass:[self class]]];
        }else{
            self.btnImageView.image = [NvBaseUtils imageNamed:self.normalString inBundle:[NSBundle bundleForClass:[self class]]];
        }
    }
}
