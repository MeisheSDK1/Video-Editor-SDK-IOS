...
- (void)viewDidLoad {
    ...
    self.themeButton = [[NvCustomButton alloc] initWithFrame:CGRectMake(49*SCREENSCALE, SCREENHEIGHT- 44 - NV_STATUSBARHEIGHT - 80*SCREENSCALE - INDICATOR, 49*SCREENSCALE, 75*SCREENSCALE)];
    self.themeButton.image = [NvUtils imageWithName:@"pip-format"];
    ...
    self.preViewButton = [[NvCustomButton alloc] initWithFrame:CGRectMake(SCREENWIDTH-2*49*SCREENSCALE, SCREENHEIGHT- 44 - NV_STATUSBARHEIGHT - 80*SCREENSCALE - INDICATOR, 49*SCREENSCALE, 75*SCREENSCALE)];
    self.preViewButton.image = [NvUtils imageWithName:@"pip-preview"];
    ...
    self.pipThemeView = [[NvPIPThemeView alloc] initWithFrame:CGRectMake(0, SCREENHEIGHT-44 - NV_STATUSBARHEIGHT-138*SCREENSCALE, SCREENWIDTH, 138*SCREENSCALE)];
    [self.view addSubview:self.pipThemeView];
    self.pipThemeView.delegate = self;
    self.pipThemeView.hidden = YES;
    
    self.firstTrackButton = [UIButton nv_buttonWithTitle:nil textColor:nil fontSize:-1 image:[NvUtils imageWithName:@"volume_up - material"]];
    [self.firstTrackButton setImage:[NvUtils imageWithName:@"volume_off - material"] forState:UIControlStateSelected];
    ...
    self.secondTrackButton = [UIButton nv_buttonWithTitle:nil textColor:nil fontSize:-1 image:[NvUtils imageWithName:@"volume_up - material"]];
    [self.secondTrackButton setImage:[NvUtils imageWithName:@"volume_off - material"]forState:UIControlStateSelected];
...
#pragma mark - 重新连接livewindow
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    [self.streamingContext connectTimeline:self.timeline withLiveWindow:self.liveWindow];
    [self.streamingContext seekTimeline:self.timeline timestamp:0 videoSizeMode:NvsVideoPreviewSizeModeLiveWindowSize flags:NvsStreamingEngineSeekFlag_ShowCaptionPoster | NvsStreamingEngineSeekFlag_ShowAnimatedStickerPoster];
}

- (void)readData {
    self.dataSource = [NSMutableArray array];

    NSString *packagePath = [[[NSBundle bundleForClass:[self class]] bundlePath] stringByAppendingPathComponent:@"PIPPackage"];
...
//    self.pipRectView.frame = topRect;
...
//    self.opView.bottom = self.pipRectView.bottom;
//    self.opView.left = self.pipRectView.left;
...
- (UIView *)leftNavigationBarItemView {
    UIButton *backButton;
    backButton = [UIButton nv_buttonWithTitle:nil textColor:nil fontSize:-1 image:[NvUtils imageWithName:@"icon_back"]];
...
- (void)initTimeline:(NvEditMode)model {
    self.editMode = model;
    self.timeline = [NvTimelineUtils createTimeline:model];
    ...
    if (isshowToast) {
        ...
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:NvLocalString(@"Tips", @"提示") message:NvLocalString(@"album.iClould", @"所选资源在iCloud中") preferredStyle:UIAlertControllerStyleAlert];
...
- (void)appendClip:(NvAlbumAsset *)obj ForTrack:(NvsVideoTrack *)track containiCloud:(BOOL *)contain {
    if (obj.asset.mediaType == PHAssetMediaTypeVideo) {
        ...
        if (localIdentifier) {
            ...
            [clip appendBuiltinFx:@"Transform 2D"];
        }
    } else {
...
- (void)adjustTowTracks:(NvsVideoTrack *)currentVideoTrack otherVideoTrack:(NvsVideoTrack *)secondVideoTrack {
    ...
    for (int i = 1; i < currentVideoTrack.clipCount; i++) {
        ...
        [clip appendPackagedFx:fxIdFirst];
    }
    ...
    for (int i = 1; i < secondVideoTrack.clipCount; i++) {
        ...
        [clip appendPackagedFx:fxIdSecond];
    }
...
- (void)cutVideo {
    ...
    NvsTimeline *timeline = [NvTimelineUtils createTimeline:NvEditMode9v16];
...
- (void)zoomOutWith:(NvsVideoTrack *)track scale:(float)scale {
    for (int i = 0; i < track.clipCount; i++) {
        ...
        if (scale1 <= 1) {
            scale1 = 1;
            [fx setFloatVal:@"Scale X" val:scale1];
            [fx setFloatVal:@"Scale Y" val:scale1];
        } else {
            [fx setFloatVal:@"Scale X" val:scale1*scale];
            [fx setFloatVal:@"Scale Y" val:scale1*scale];
        }
...
- (void)nvPIPThemeView:(NvPIPThemeView *)pipThemeView applyTemplate:(NvPIPThemeItem *)item {
    self.selectIndex = [self.dataSource indexOfObject:item];
