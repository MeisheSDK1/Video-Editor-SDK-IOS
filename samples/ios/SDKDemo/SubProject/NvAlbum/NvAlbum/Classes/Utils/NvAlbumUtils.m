...
+ (NSString *)getImagePath:(NSString *)name scale:(NSInteger)scale{
    ...
    //返回删掉旧名称加上新名称的路径
    // Returns the path with the old name removed and the new name added
    ...
    if (@available(iOS 9, *)) {
        ...
        [resourceList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([resource.uniformTypeIdentifier isEqualToString:@"com.compuserve.gif"]) {
                ...
                // stitching images called resource. OriginalFilename path
...
+ (NSString *)convertIntegerToFormatString:(NSUInteger)number {
    NSNumber *num = [NSNumber numberWithUnsignedInteger:number];
