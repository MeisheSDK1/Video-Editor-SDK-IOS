...
@interface JLLewReorderableLayout ()
...
@implementation JLLewReorderableLayout
...
- (void)moveItemIfNeeded {
    ...
    [self.collectionView performBatchUpdates:^{
        ...
        [self.collectionView reloadData];if ([_delegate respondsToSelector:@selector(collectionView:itemAtIndexPath:didMoveToIndexPath:)]) {
    ...
    if (_cellFakeView != nil) {
        switch (pan.state) {
            case UIGestureRecognizerStateChanged:{
                ...
                center.y = self.fakeCellCenter.y + self.panTranslation.y;
                _cellFakeView.center = center;
                CGFloat y = self.collectionView.frame.size.height;

                if (center.y >= self.collectionView.frame.size.height - 80.0) {
                
                    NSIndexPath *indexPath = _cellFakeView.indexPath;
                    [self invalidateDisplayLink];
                    if ([_delegate respondsToSelector:@selector(collectionView:layout:didDeleteItemAtIndexPath:)]) {
                        [_delegate collectionView:self.collectionView layout:self didDeleteItemAtIndexPath:indexPath];
                    }
                    [_cellFakeView removeFromSuperview];
                    _cellFakeView = nil;
                }
