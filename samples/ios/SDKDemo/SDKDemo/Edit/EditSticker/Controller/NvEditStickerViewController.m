...
#import "NvEditStickerKeyFrameView.h"
#import "NvGraphicBtn.h"
#import "NvStickerAnimationController.h"

#define NV_DEFAULT_STICKER_DURATION 5000000

@interface NvEditStickerViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,NvsCTimelineEditorDelegate, NvRectViewDelegate, NvTimeLabelViewDelegate, NvsStreamingContextDelegate, NvLiveWindowPanelViewDelegate,NvCafCreatorDelegate,NvEditStickerKeyFrameViewDelegate>
...
/// 关键帧视图
/// Keyframe view
@property (nonatomic, strong) NvEditStickerKeyFrameView *keyFrameView;
...
- (void)addSubViews {
    ...
    self.playBtn = [[NvButton alloc] initWithFrame:CGRectMake(0, 40*SCREENSCALE, 40*SCREENSCALE, 49*SCREENSCALE)];
...
-(void)animationButtonClick:(UIButton *)btn{
    NvStickerAnimationController *VC = [[NvStickerAnimationController alloc] init];
...
- (void)initTimeline {
    ...
    for (NvStickerInfoModel *info in self.stickerInfoArray) {
        for (NvKeyFrameStickerModel *info2 in info.keyFramesArray) {
            NSLog(@"%lld====%@",info2.time,NSStringFromCGPoint(info2.translation));
        }
        NSLog(@"%lld----%lld",info.inPoint,info.outPoint);
    }
...
- (void)onFinishSelectSticker {
    ...
    ///重新从选择位置处开始播放
    ///Restart playback from the selected position
...
- (void)keyFrameButtonClicked:(UIButton *)button {
    ...
    if (!self.keyFrameView) {
        self.keyFrameView = [[NvEditStickerKeyFrameView alloc]initWithFrame:CGRectMake(0, SCREENHEIGHT - 104*SCREENSCALE-INDICATOR, SCREENWIDTH, 104*SCREENSCALE+INDICATOR)];
...
#pragma mark 一键清除当前贴纸关键帧 One key to clear the current sticker key frame
...
- (void)seekTimeline:(int64_t)postion {
    ...
    if (self.isChange) {
        ...
        CGFloat scaleForSeek = self.timeline.duration / 1000000 /  [self.timelineEditor getTimelineEditorWidth] / UIScreen.mainScreen.scale;
        [self.streamingContext setTimeline:_timeline scaleForSeek:scaleForSeek];
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self refreshRectViewUnderNOKeyframeState];
}
...
- (void)refreshRectViewUnderNOKeyframeState {
    if (self.currentSticker != nil && [self underShowKeyFrameViewState] == NO) {
        int64_t pos = [self.streamingContext getTimelineCurrentPosition:self.timeline];
...
- (void)updateAllStickers {
    ...
    NSArray *array = [self.assetManager getUsableAssets:ASSET_ANIMATED_STICKER aspectRatio:[NvSDKUtils convertRatio:self.editMode] categoryId:NV_CATEGORY_ID_ALL kindId:NV_KIND_ID_ALL];
...
- (void)nvAlbumViewController:(NvAlbumViewController *)albumViewController selectAlbumAssets:(NSMutableArray <NvAlbumAsset *>*)assets {
    ...
    options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
    options.networkAccessAllowed = YES;
    [[PHImageManager defaultManager] requestImageDataForAsset:assetsGif.asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
...
- (void)cafCreator:(nonnull NvCafCreator *)creator convertUIImage:(nonnull UIImage *)image {
...
- (void)timelineEditor:(id)timelineEditor dragScrollTimelineEnded:(int64_t)timestamp {
    ...
    if ([self underShowKeyFrameViewState]) {
        ...
        self.currentSticker = [[self.timeline getAnimatedStickersByTimelinePosition:timestamp] lastObject];
...
#pragma mark 代理NvEditStickerKeyFrameViewDelegate
/**
    ...
    if (self.selectPanelView.hidden) {
        ...
        if ([self underShowKeyFrameViewState]) {
            ...
            if(self.currentSticker.inPoint <= position && self.currentSticker.outPoint >= position && self.currentSticker) {
                ...
                self.currentSticker = [[self.timeline getAnimatedStickersByTimelinePosition:position] lastObject];
            }
...
- (void)didStreamingEngineStateChanged:(NvsStreamingEngineState)state {
    if (self.selectPanelView.hidden) {
        ...
        if (self.currentIndexPath) {
            stickerInfo = self.currentStickerArray[self.currentIndexPath.item];
        }
        if (state != NvsStreamingEngineState_Playback) {
            [self.playBtn setImage:[UIImage imageNamed:@"NvPlayback"] forState:UIControlStateNormal];
            stickerInfo.isPlay = NO;
