...
@interface NvEditBGColorView ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation NvEditBGColorView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.collectionView registerClass:[NvEditBGColorCell class] forCellWithReuseIdentifier:@"NvEditBGColorCellID"];
        [self addSubview:self.collectionView];
        [self.collectionView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.mas_equalTo(0);
            make.height.mas_equalTo(32.0*SCREENSCALE);
...
- (void)configData:(NSArray *)array {
    self.colorArr = [NSMutableArray arrayWithArray:array];
    [self.collectionView reloadData];
}

//完成按钮点击方法 Complete the button click method
- (void)applyButtonClicked:(UIButton *)button {
    if ([self.delegate respondsToSelector:@selector(nvEditBGColorView:applyButtonClicked:)]) {
        [self.delegate nvEditBGColorView:self applyButtonClicked:button];
    }
}

//点击应用全部界面方法 Click Apply All Interface methods
- (void)tapApplyAllMethod:(UITapGestureRecognizer *)sender {
    NSLog(@"应用全部 Apply all");
    if ([self.delegate respondsToSelector:@selector(nvEditBGColorViewApplyAll:)]) {
        [self.delegate nvEditBGColorViewApplyAll:self];
    }
}
...
{
    NvEditBGColorCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NvEditBGColorCellID" forIndexPath:indexPath];
    cell.model = self.colorArr[indexPath.item];
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(nvEditBGColorView:selectModel:)]) {
        NvEditBGColorModel *model = self.colorArr[indexPath.item];
        [self.delegate nvEditBGColorView:self selectModel:model];
    }
}
...
-(UICollectionViewFlowLayout *)flowLayout{
    if (_flowLayout == nil){
        ...
        _flowLayout.itemSize = CGSizeMake(20.0*SCREENSCALE, 30.0*SCREENSCALE);
