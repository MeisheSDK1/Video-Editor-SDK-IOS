...
@interface NvEditMicroShapeEffectView()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation NvEditMicroShapeEffectView
- (instancetype)init {
    if (self = [super init]) {
...
#pragma mark - 所有效果全部置为零 或 恢复原有效果
//Set all effects to zero or restore original effects
- (void)reapplyAppliedEffects:(BOOL)needRefreshView refreshData:(BOOL)needRefreshData {
    if ([self.delegate respondsToSelector:@selector(nvEditMicroShapeEffectView:selecteModel:refreshView:refreshData:)]) {
        for(NvBeautyTypeModel *model in self.dataSource) {
            if (needRefreshView && [model.fxName isEqualToString:self.currentModel.fxName]) {
                [self.delegate nvEditMicroShapeEffectView:self selecteModel:model refreshView:YES refreshData:YES];
            }else{
                [self.delegate nvEditMicroShapeEffectView:self selecteModel:model refreshView:NO refreshData:needRefreshData];
            }
        }
    }
}
        ...
        for(NvBeautyTypeModel *model in self.originalDataSource) {
            ...
            for(NvBeautyTypeModel *item in self.dataSource) {
                if ([model.name isEqualToString:item.name]) {
                    ...
                    newM.uuid = item.uuid;
                    newM.canReplace = item.canReplace;
                    break;
                }
            }
            [self.delegate nvEditMicroShapeEffectView:self selecteModel:newM refreshView:needRefreshView refreshData:needRefreshData];
        }
    }
}
...
- (void)changeSwitchView:(NvSwitchView *)sender {
    ...
    if(sender.selected){
        ...
        label.text = NvLocalString(@"Close micro shaping", @"关闭微整形");
        ...
        label.text = NvLocalString(@"Open micro shaping", @"开启微整形");
        [UIView animateWithDuration:0.1 animations:^{
...
- (void)beautyBtnClick:(NvSwitchView *)sender {
    ...
    [self.collectionView reloadData];
    if ([self.delegate respondsToSelector:@selector(nvEditMicroShapeEffectView:switchMicroShapeSum:)]) {
        [self.delegate nvEditMicroShapeEffectView:self switchMicroShapeSum:sender.selected];
    }
}
...
- (void)resetBtnClick:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(nvEditMicroShapeEffectView:selecteModel:refreshView:refreshData:)]) {
        if (self.dataSource.count > 0) {
            ...
            for(NvBeautyTypeModel *model in self.dataSource) {
                [self.delegate nvEditMicroShapeEffectView:self selecteModel:model refreshView:NO refreshData:NO];
            }
        }

        if (self.currentModel) {
            [self.delegate nvEditMicroShapeEffectView:self selecteModel:self.currentModel refreshView:YES refreshData:YES];
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!model.canReplace) {
        if ([self.delegate respondsToSelector:@selector(nvEditMicroShapeEffectView:forbiddenReplace:)]) {
            [self.delegate nvEditMicroShapeEffectView:self forbiddenReplace:model];
        }
        return;
    }
    for (NvBeautyTypeModel *model in self.dataSource) {
    ...
    model.selected = YES;
    if (![self.currentModel.name isEqualToString:model.name] && [self.delegate respondsToSelector:@selector(nvEditMicroShapeEffectView:selecteModel:refreshView:refreshData:)]) {
        [self.delegate nvEditMicroShapeEffectView:self selecteModel:model refreshView:YES refreshData:YES];
    }
